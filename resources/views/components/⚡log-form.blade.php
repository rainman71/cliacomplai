<?php

use App\Forms\FormCatalog;
use App\Models\Lab;
use App\Services\FormService;
use App\Support\CurrentLab;
use Livewire\Attributes\Computed;
use Livewire\Component;

/**
 * Generic catalog-driven LOG form: a set of header fields plus a repeating data-entry table whose
 * columns are declared in FormCatalog (`columns`). Used for the numeric/tabular Rightsize SOPs that
 * are lists of entries rather than a single questionnaire (calibration, pipette, credentials, etc.).
 * Stores answers as ['fields' => header values, 'rows' => [[col => value], ...]] and completes the
 * linked obligation, exactly like ⚡form-wizard.
 */
new class extends Component
{
    public int $labId;
    public string $labName = '';
    public string $formCode = '';

    public array $values = []; // header field key => value
    public array $rows = [];   // [ [col => value], ... ]
    public ?int $lastResponseId = null;

    public function mount(Lab $lab, string $code): void
    {
        abort_unless(auth()->user()?->canEditLab($lab), 403);
        $def = FormCatalog::get($code);
        abort_unless(($def['component'] ?? null) === 'log-form', 404);

        $this->labId = $lab->id;
        $this->labName = $lab->name;
        $this->formCode = $code;
        app(CurrentLab::class)->set($lab);

        foreach ($def['fields'] ?? [] as $f) {
            $this->values[$f['key']] = $this->resolveDefault($lab, (string) ($f['default'] ?? ''));
        }
        $this->rows = [$this->blankRow()];
    }

    public function hydrate(): void
    {
        $lab = Lab::find($this->labId);
        abort_unless($lab && auth()->user()?->canEditLab($lab), 403);
        app(CurrentLab::class)->set($lab);
    }

    private function blankRow(): array
    {
        $row = [];
        foreach (FormCatalog::get($this->formCode)['columns'] ?? [] as $c) {
            $row[$c['key']] = '';
        }

        return $row;
    }

    public function addRow(): void
    {
        $this->rows[] = $this->blankRow();
    }

    public function removeRow(int $i): void
    {
        unset($this->rows[$i]);
        $this->rows = array_values($this->rows);
        if (empty($this->rows)) {
            $this->rows = [$this->blankRow()];
        }
    }

    private function resolveDefault(Lab $lab, string $token): string
    {
        return match (true) {
            $token === 'today' => now()->toDateString(),
            $token === 'lab_name' => (string) $lab->name,
            $token === 'lab_clia' => (string) ($lab->clia_number ?? ''),
            str_starts_with($token, 'profile:') => $lab->profileValue(substr($token, 8)),
            default => $token,
        };
    }

    #[Computed]
    public function def(): array
    {
        return FormCatalog::get($this->formCode);
    }

    public function submit(FormService $forms): void
    {
        abort_unless(auth()->user()?->canEditLab(Lab::find($this->labId)), 403);

        $def = FormCatalog::get($this->formCode);

        // Drop rows where every column is blank so a trailing empty line isn't filed.
        $rows = array_values(array_filter($this->rows, fn ($r) => trim(implode('', $r)) !== ''));

        $dateField = $def['date_field'] ?? null;
        $completedDate = $dateField ? ($this->values[$dateField] ?? null) : null;

        $response = $forms->submit($this->formCode, [
            'fields' => $this->values,
            'rows' => $rows,
        ], $completedDate ?: null);

        $this->lastResponseId = $response->id;
        session()->flash('formSaved', $def['title'].' filed — the linked obligation is marked complete.');
    }
}; ?>

<div class="min-h-screen bg-gray-50 text-gray-900">
    <div class="mx-auto max-w-4xl px-4 py-6">
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold tracking-tight">{{ $this->def['title'] }}</h1>
                <p class="text-sm text-gray-500">{{ $this->def['sop_code'] ?? '' }} · {{ $labName }}</p>
            </div>
            <a href="{{ route('dashboard', $labId) }}" class="text-sm font-medium text-indigo-600 hover:underline">← Back to dashboard</a>
        </div>

        @if (session('formSaved'))
            <div class="mb-4 rounded-md bg-green-50 p-3 text-sm text-green-800 ring-1 ring-green-200">
                {{ session('formSaved') }}
                @if ($lastResponseId)
                    <a href="{{ route('forms.pdf', ['lab' => $labId, 'response' => $lastResponseId]) }}" class="ml-2 font-medium underline">Download the completed PDF</a>
                @endif
            </div>
        @endif

        @if (! empty($this->def['instructions']))
            <p class="mb-4 rounded-md bg-amber-50 p-3 text-xs text-amber-800 ring-1 ring-amber-200">{{ $this->def['instructions'] }}</p>
        @endif

        <form wire:submit="submit" class="space-y-6">
            @if (! empty($this->def['fields']))
                <div class="grid grid-cols-1 gap-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:grid-cols-2">
                    @foreach ($this->def['fields'] as $f)
                        <label class="block text-sm"><span class="font-medium text-gray-700">{{ $f['label'] }}</span>
                            @if (($f['type'] ?? 'text') === 'select')
                                <select wire:model="values.{{ $f['key'] }}" class="mt-1 w-full rounded-md border-gray-300 text-sm shadow-sm">
                                    <option value="">— select —</option>
                                    @foreach ($f['options'] ?? [] as $opt)
                                        <option value="{{ $opt }}">{{ $opt }}</option>
                                    @endforeach
                                </select>
                            @else
                                <input type="{{ ($f['type'] ?? 'text') === 'date' ? 'date' : 'text' }}" wire:model="values.{{ $f['key'] }}" class="mt-1 w-full rounded-md border-gray-300 text-sm shadow-sm">
                            @endif
                        </label>
                    @endforeach
                </div>
            @endif

            <div class="space-y-3">
                @foreach ($rows as $i => $row)
                    <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                        <div class="mb-2 flex items-center justify-between">
                            <span class="text-xs font-semibold uppercase tracking-wide text-gray-400">Entry #{{ $i + 1 }}</span>
                            <button type="button" wire:click="removeRow({{ $i }})" class="text-xs font-medium text-red-600 hover:underline">Remove</button>
                        </div>
                        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                            @foreach ($this->def['columns'] as $c)
                                <label class="block text-sm"><span class="font-medium text-gray-700">{{ $c['label'] }}</span>
                                    <input type="text" wire:model="rows.{{ $i }}.{{ $c['key'] }}" class="mt-1 w-full rounded-md border-gray-300 text-sm shadow-sm"></label>
                            @endforeach
                        </div>
                    </div>
                @endforeach
                <button type="button" wire:click="addRow" class="rounded-md border border-gray-300 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 shadow-sm hover:bg-gray-50">+ Add another entry</button>
            </div>

            <div class="flex items-center justify-end gap-3">
                <span class="text-xs text-gray-400">Filing this marks the linked obligation complete and advances its due date.</span>
                <button type="submit" class="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">Complete &amp; file</button>
            </div>
        </form>
    </div>
</div>
